...
const BlogSection = () => {
  ...
  return (
    <section id="blog" className="pt-20 px-4 bg-[var(--main-bg-color)] flex flex-col items-center">
      {/* Header */}
      <div className="text-center mb-8">
        <h2 className="text-3xl font-semibold text-[var(--main-text-color)]">Blog</h2>
        <p className="open-sans-200 text-md text-[var(--secondary-text-color)] mt-2">
          Exploring design, code, and growth
        </p>
      </div>
      ...
      {loading ? (
        ...
        <>
              ...
              {filteredPosts.map((post)=>(
                <motion.div key={post.slug.current} initial={{opacity:0,y:20}} animate={{opacity:1,y:0}}
                  ...
                  <div className="p-6 flex flex-col flex-grow">
                    <p className="text-xs text-gray-500 mb-2">{new Date(post.date).toLocaleDateString()} • {post.category}</p>
                    <h3 className="text-lg font-semibold mb-3">{post.title}</h3>
                    <p className="text-sm mb-5 flex-grow">{post.excerpt}</p>
                    <Link to={`/blog/${post.slug.current}`} className="px-3 py-2 bg-[var(--accent)] text-[var(--main-bg-color)] rounded-md">Read more →</Link>
                  </div>
                </motion.div>
              ))}
            </AnimatePresence>
          ...
          {/* Explore More only if posts exist */}
          <div className="mt-10">
            <Link to="/blog" className="px-5 py-3 bg-[var(--accent)] text-[var(--main-bg-color)] rounded-md">
